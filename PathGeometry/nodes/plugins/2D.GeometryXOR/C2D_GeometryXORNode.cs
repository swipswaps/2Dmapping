...
{
	#region PluginInfo
	[PluginInfo(Name = "XOR", Category = "2D.Geometry", Help = "Basic template with one value in/out", Tags = "")]
	#endregion PluginInfo
	public class C2D_GeometryXORNode : IPluginEvaluate
	{
		#region fields & pins
		[Input("Input 1")]
		...
		public void Evaluate(int SpreadMax)
		{
			...
			for (int i = 0; i < SpreadMax; i++) {
				try {
					PathGeometry geom = PathGeometry.Combine(FGeometryIn[i], FGeometryIn2[i], GeometryCombineMode.Xor, null);
